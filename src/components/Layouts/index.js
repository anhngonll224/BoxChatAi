...
import "./styles.scss"
import NoticeCreateRequest from "../NoticeCreateRequest"
import {
  changePercent,
  changeTitleMessage,
  setMinPercent,
  setShowStepEnd,
  showNoticeCreate,
} from "src/redux/noticeCreateRequest"
import { setDataSigned, setListDataSigned } from "src/redux/dataSign"
...
const MainLayout = ({ children, isAdmin, isUser }) => {
  ...
  const { showNotice, percent } = useSelector(
    state => state.noticeCreateRequest,
  )
  ...
  useEffect(() => {
    if (!!connection) {
      ...
      //Loading góc phải
      connection.on("InfoCreateRequestOrder", message => {
        dispatch(changeTitleMessage(message.message))
        if (message.data.percent >= percent) {
          dispatch(changePercent(message.data.percent))
        }
        dispatch(setShowStepEnd(message.data.isShowStep4))
        if (message.data.isOk && message.data.percent) {
          dispatch(setMinPercent(72))
          dispatch(showNoticeCreate(true))
        } else if (!message.data.isOk) {
          dispatch(showNoticeCreate(false))
          Notice({ msg: message.message, isSuccess: false })
        }
      })

      //Loading Api
      connection.on("Api", message => {
        console.log("message", message)
        if (message?.data?.Type === 1) {
          dispatch(setDataSigned(message?.data?.Data?.ca_list))
        }
        if (message?.data?.Type === 2) {
          dispatch(setListDataSigned(message?.data?.Data?.data))
        }
      })
    }
  }, [connection])
                    ...
                    >
                      ...
                      >
                        ...
                        <Col
                          ...
                          className={`d-flex-center nowrap mr-20style avata ${!isAdmin && "pointer"
                            } `}
                          style={{
                            whiteSpace: "nowrap",
                            height: "40px",
                        ...
                        >
                          {!!userInfo?.FileUrl && !!isAdmin ? (
                            ...
                            <div
                              className={`name-branch h-100pe ${isAdmin ? "" : "hover-menu"
                                }`}
                              onClick={() => {
                            ...
                            >
                              <span
                                className={`fw-600 d-flex-center h-100pe ${!!isMobile ? "fs-12" : "fs-20"
                                  }`}
                              >
                                <img src={Logo} className="logo " />
                                {!isMobile && (
                                  <div className="h-100pe d-flex-center logo-text">
                                    Hệ thống quản lý tên định danh quốc gia 123
                                  </div>
                                )}
                              </span>
                            </div>
                    ...
                    >
                      ...
                      >
                        ...
                        <Row
                          ...
                          className={`align-items-center nowrap mr-20style  ${"pointer"} `}
                          style={{
                            whiteSpace: "nowrap",
                            height: "40px",
                            paddingLeft: 0,
